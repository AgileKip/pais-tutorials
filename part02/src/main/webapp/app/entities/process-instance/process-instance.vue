<template>
  <div>
    <h2 id="page-heading" data-cy="ProcessInstanceHeading">
      <span v-text="$t('travelPlanProcessApp.processInstance.home.title')" id="process-instance-heading">Process Instances</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('travelPlanProcessApp.processInstance.home.refreshListLabel')">Refresh List</span>
        </button>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && processInstances && processInstances.length === 0">
      <span v-text="$t('travelPlanProcessApp.processInstance.home.notFound')">No processInstances found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="processInstances">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('travelPlanProcessApp.processInstance.businessKey')">Business Key</span></th>
            <th scope="row"><span v-text="$t('travelPlanProcessApp.processInstance.status')">Status</span></th>
            <th scope="row"><span v-text="$t('travelPlanProcessApp.processInstance.startDate')">Start Date</span></th>
            <th scope="row"><span v-text="$t('travelPlanProcessApp.processInstance.endDate')">End Date</span></th>
            <th scope="row"><span v-text="$t('travelPlanProcessApp.processInstance.camundaDeploymentId')">Camunda Deployment Id</span></th>
            <th scope="row">
              <span v-text="$t('travelPlanProcessApp.processInstance.camundaProcessDefinitionId')">Camunda Process Definition Id</span>
            </th>
            <th scope="row">
              <span v-text="$t('travelPlanProcessApp.processInstance.camundaProcessInstanceId')">Camunda Process Instance Id</span>
            </th>
            <th scope="row">
              <span v-text="$t('travelPlanProcessApp.processInstance.camundaProcessVariables')">Camunda Process Variables</span>
            </th>
            <th scope="row"><span v-text="$t('travelPlanProcessApp.processInstance.processDefinition')">Process Definition</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="travelPlanProcessApp.processInstance.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-processInstance-heading" v-text="$t('travelPlanProcessApp.processInstance.delete.question', { id: removeId })">
